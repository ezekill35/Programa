...
import { collection, addDoc, onSnapshot, doc, deleteDoc, query, where, getDocs } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
onSnapshot(colProveedores,snap=>{
  ...
  snap.forEach(docu=>{
    ...
    tr.innerHTML=`<td>${p.ruc}</td><td>${p.nombre}</td><td>${p.direccion||"-"}</td><td>${p.telefono||"-"}</td><td><button class="btn eliminar" data-id="${docu.id}" data-tipo="proveedor">🗑</button></td>`;
...
onSnapshot(colProductos,snap=>{
  ...
  snap.forEach(docu=>{
    const p=docu.data();
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${p.nombre}</td><td>${p.cantidad}</td><td>${p.precio}</td><td>${p.descripcion||"-"}</td><td><button class="btn eliminar" data-id="${docu.id}" data-tipo="producto">🗑</button></td>`;
    tablaProductos.appendChild(tr);

    const opt=document.createElement("option"); opt.value=p.nombre; opt.textContent=p.nombre;
...
onSnapshot(colFacturas,snap=>{
  ...
  snap.forEach(docu=>{
    const f=docu.data();
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${f.idFactura}</td><td>${f.fecha}</td><td>${f.proveedor}</td><td>${f.producto}</td><td>${f.monto}</td><td>${f.tipo}</td><td><button class="btn eliminar" data-id="${docu.id}" data-tipo="factura">🗑</button></td>`;
