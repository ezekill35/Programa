...
const cerrarEditarLista = document.getElementById("cerrarEditarLista");
...
onSnapshot(colProveedores, snapshot=>{
  ...
  snapshot.forEach(docu=>{
    ...
    tr.innerHTML = `
      ...
      <td><button class="btn-accion editar" data-id="${docu.id}" data-tipo="proveedor">✏️</button></td>`;
...
onSnapshot(colProductos, snapshot=>{
  ...
  snapshot.forEach(docu=>{
    ...
    tr.innerHTML = `
      ...
      <td><button class="btn-accion editar" data-id="${docu.id}" data-tipo="producto">✏️</button></td>`;
...
// ===================== MODAL EDICIÓN =====================
function abrirModalEdicion(tipo,id,data){
  editarId = id;
  editarTipo = tipo;
  camposEditar.innerHTML = "";
  modalEditarTitulo.textContent = tipo.charAt(0).toUpperCase() + tipo.slice(1);
  for(const key in data){
    const value = data[key];
    const div = document.createElement("div");
    div.className = "col-12 mb-2";
    if(typeof value==="string" && value.length>40){
      div.innerHTML = `<textarea class="form-control" name="${key}" rows="3">${value}</textarea>`;
    } else {
      div.innerHTML = `<input type="text" class="form-control" name="${key}" value="${value}">`;
    }
    camposEditar.appendChild(div);
  }
  modalEditarLista.showModal();
}

formEditarLista.addEventListener("submit", async e=>{
  e.preventDefault();
  if(!editarId || !editarTipo) return;
  const formData = new FormData(e.target);
  const data = {};
  formData.forEach((v,k)=>{
    const val = isNaN(v) ? v : parseFloat(v);
    data[k] = val;
  });
  await updateDoc(doc(db, editarTipo==="proveedor"?"proveedores":editarTipo==="producto"?"productos":"facturas", editarId), data);
  modalEditarLista.close();
  alert("Registro actualizado ✅");
});
cerrarEditarLista.addEventListener("click", ()=> modalEditarLista.close());
...
document.addEventListener("click", async e=>{
  ...
  if(e.target.classList.contains("editar") && tr){
    ...
    abrirModalEdicion(tipo,id,data);
  }

  // Ver info proveedor/producto/factura
  if(e.target.classList.contains("link-info")){
    const tipo = e.target.dataset.tipo;
    const id = e.target.dataset.id;
    if(!confirm(`¿Deseas ver la información de este ${tipo}?`)) return;
    const docSnap = await getDoc(doc(db,tipo==="proveedor"?"proveedores":tipo==="producto"?"productos":"facturas",id));
    if(!docSnap.exists()){ alert(`${tipo} no encontrado`); return; }
    if(tipo==="factura") mostrarModalFactura(docSnap.data());
    else {
      const d = docSnap.data();
      modalExtraBody.innerHTML = tipo==="proveedor"?
        `<h5>Proveedor</h5><p><b>RUC:</b>${d.ruc}</p><p><b>Nombre:</b>${d.nombre}</p><p><b>Dirección:</b>${d.direccion||""}</p><p><b>Teléfono:</b>${d.telefono||""}</p>`:
        `<h5>Producto</h5><p><b>Nombre:</b>${d.nombre}</p><p><b>Cantidad:</b>${d.cantidad}</p><p><b>Precio:</b>${d.precio}</p><p><b>Descripción:</b>${d.descripcion||""}</p>`;
