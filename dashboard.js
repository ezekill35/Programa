...
onSnapshot(colProductos, snap => {
  ...
  snap.forEach(d => {
    ...
    tr.innerHTML = `
      ...
      <td>${data.moneda} ${data.precio.toFixed(2)}</td>
...
formFactura.addEventListener("submit", async e => {
  ...
  const data = {
    ...
    igv: parseFloat((document.getElementById("montoFactura").value)*0.18),
    total: parseFloat((document.getElementById("montoFactura").value)*1.18),
...
function mostrarModalFactura(f){
  modalFacturaBody.innerHTML = `
    ...
    <p><b>Subtotal:</b> ${f.subtotal.toFixed(2)}</p>
    <p><b>IGV:</b> ${f.igv.toFixed(2)}</p>
