...
  e.preventDefault();
  // 💡 CORRECCIÓN 2: Capturando rucProveedor y tipoDocumentoProveedor (select)
  const data = {
...
    // 💡 CORRECCIÓN 3: Mostrando tipoDocumento y ruc
    tr.innerHTML=`
...
    // Añadida presentacionProducto al objeto data
...
    tr.innerHTML=`
      <td>${d.nombre}</td>
      <td>${d.presentacion || ''} (${d.cantidad})</td>       <td>${d.moneda === 'soles' ? 'S/. ' : '$ '}${d.precio}</td> 
...
// Generar ID de factura (simulación)
...
    subtotal: monto, // Cambiado 'monto' a 'subtotal' para mayor claridad
    igv: igv, 
    total: monto + igv, // Agregado total
    tipo: document.getElementById("tipoFactura").value,
    // La moneda se obtiene del form de producto, pero como está en la factura, usamos un valor por defecto o la del producto si se pudiera obtener
    moneda: document.getElementById("tipoMoneda").value || "soles" 
  };
  await addDoc(colFacturas, data);
  formFactura.reset();
  document.getElementById("idFactura").value = generarIdFactura(); // Generar nuevo ID
});
...
      <td>${f.proveedor}</td>
      <td>${f.producto}</td>
      <td>${f.moneda === 'soles' ? 'S/. ' : '$ '}${f.subtotal?.toFixed(2) || f.monto?.toFixed(2) || '0.00'}</td>
      <td>${f.moneda === 'soles' ? 'S/. ' : '$ '}${f.igv?.toFixed(2) || '0.00'}</td>
      <td>${f.moneda === 'soles' ? 'S/. ' : '$ '}${f.total?.toFixed(2) || '0.00'}</td>
      <td>${f.tipo}</td>
      <td>
...
      div.textContent=f.idFactura + " - " + f.producto; // Mostrar más info
...
// ===================== CLICK GLOBAL (Editar/Eliminar/Ver) =====================
...
          <label>RUC/Documento</label><input id="editRuc" class="form-control mb-1" value="${d.ruc||''}">
...
        tipo==="producto"?`
          <label>Nombre</label><input id="editNombre" class="form-control mb-1" value="${d.nombre||''}">
          <label>Presentación</label><input id="editPres" class="form-control mb-1" value="${d.presentacion||''}">
...
          <label>Subtotal</label><input id="editMonto" type="number" step="0.01" class="form-control mb-1" value="${d.subtotal||d.monto||0}">
...
          upd.nombre=document.getElementById("editNombre").value.trim();
          upd.presentacion=document.getElementById("editPres").value.trim();
...
          const editMonto = parseFloat(document.getElementById("editMonto").value);
          const igv = editMonto * 0.18;
...
          upd.subtotal=editMonto;
          upd.igv=igv;
          upd.total=editMonto+igv;
...
    const idFactura = e.target.dataset.id; // Para facturas

    if(tipo === "factura"){
      const snap = await getDocs(query(colFacturas,where("__name__","==",idFactura)));
...
          <h5>${tipo} ${nombre}</h5>
          <p>${tipo==="proveedor"?`Tipo Documento: ${d.tipoDocumento}<br>RUC/Documento: ${d.ruc}<br>Dirección: ${d.direccion||''}<br>Tel: ${d.telefono||''}`:
          `Presentación: ${d.presentacion} (${d.cantidad})<br>Precio: ${d.moneda === 'soles' ? 'S/. ' : '$ '}${d.precio}<br>Descripción: ${d.descripcion||''}`}</p>
        `;
