...
async function eliminarProveedorFirebase(id){ if(confirm('¿Eliminar proveedor?')) await deleteDoc(doc(db,"proveedores",id)); }
...
async function eliminarFacturaFirebase(id){ if(confirm('¿Eliminar factura?')) await deleteDoc(doc(db,"facturas",id)); }
...
async function eliminarGastoFirebase(id){ if(confirm('¿Eliminar gasto?')) await deleteDoc(doc(db,"gastos",id)); }
  ...
  servicios.forEach(s=>{
    listaServicios.innerHTML+=`
      <tr>
        ...
        <td>${s.precio}</td>
        <td>
          <button class="btn btn-sm btn-primary me-1" onclick="editarServicio('${s.id}')">✏️</button>
...
async function eliminarServicioFirebase(id){ if(confirm('¿Eliminar servicio?')) await deleteDoc(doc(db,"servicios",id)); }

// ------------------ Cerrar sesión ------------------
document.getElementById('menu-logout').addEventListener('click', ()=>{
  signOut(auth).then(()=> window.location.href='index.html').catch(err=>alert('Error: '+err.message));
});
